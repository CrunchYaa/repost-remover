...
interface Settings { scanIntervalMinutes: number; autoRemove: boolean; notifications: boolean; notifyOnRemoval: boolean; }
const settings: Settings = { scanIntervalMinutes: 5, autoRemove: true, notifications: true, notifyOnRemoval: true };
...
router.patch("/settings", async (req, res) => {
  ...
  if (updates.scanIntervalMinutes !== undefined) { const { setAutomationInterval } = await import("./automation.js"); setAutomationInterval(updates.scanIntervalMinutes); }
  res.json(settings);
});
