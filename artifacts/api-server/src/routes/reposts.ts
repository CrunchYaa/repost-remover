...
function initDailyData() {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    dailyData.push({
      ...
      count: i === 0 ? 0 : Math.floor(Math.random() * 12),
    });
  }
}
...
router.post("/reposts/scan", async (req, res) => {
  ...
  for (let i = 0; i < newCount; i++) {
    ...
    if (!isDuplicate) {
      reposts.unshift(generateFakeRepost());
      newFound++;
    }
  }
  ...
  addLog(
    "scan",
    `Scan completed — found ${newFound} new repost${newFound !== 1 ? "s" : ""}`,
    `Total pending: ${reposts.filter((r) => r.status === "pending").length}`,
  );

  res.json({
    found: reposts.filter((r) => r.status === "pending").length,
    newFound,
    scannedAt,
  });
});
...
router.post("/reposts/remove-all", async (req, res) => {
  ...
  if (removed > 0) {
    addLog(
      "removal",
      `Removed ${removed} repost${removed !== 1 ? "s" : ""}`,
...
router.delete("/reposts/:id", async (req, res) => {
  ...
  if (!parsed.success) {
    res.status(400).json({ error: "Invalid repost id" });
    return;
  }
  ...
  if (Math.random() > 0.05) {
    repost.status = "removed";
    incrementRemoved();
    addLog("removal", `Removed repost from ${repost.originalAuthor}`);
  } else {
    repost.status = "failed";
    addLog("error", `Failed to remove repost from ${repost.originalAuthor}`);
  }
